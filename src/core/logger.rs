...
pub struct RaftLogger(LoggingLevel);

///Define the different levels for log messages
...
impl log::Log for RaftLogger {
    ...
    fn log(&self, record: &log::Record<'_>) {
        ...
        ///Don't print Hyper or Results messages unless debug is enabled.
        let configged_level = self.0;
        let from_hyper = record
            .module_path()
            .map_or(false, |m| m.starts_with("hyper::"));
        let from_results = record
            .module_path()
            .map_or(false, |m| m.starts_with("rustls::"));
        if configged_level != LoggingLevel::Debug && (from_hyper || from_results) {
            return;
        }
        ///Abuse targets with suffix "_" to indicate indentation
...
crate fn try_init(level: LoggingLevel, verbose: bool) -> bool {
    ...
    if let Err(e) = log::set_boxed_logger(Box::new(RaftLogger(level))) {
        if verbose {
...
fn usize_to_filter(num: usize) -> log::LevelFilter {
    match num {
        ...
        _ => unreachable!("max num is 5 in filiter_to_usize"),
    }
}
...
/// Expose logging macros as (hidden) functions for use by core/contrib codegen
